...
// --- PENAMBAHAN ICON BARU (GoFood, ShopeeFood, GrabFood) ---

// GoFood Logo (MENGGUNAKAN SVG RESMI GOJEK)
const GoFoodLogo = (props) => (
...
// ShopeeFood Logo (Menggunakan SVG Resmi, disesuaikan agar ukurannya lebih kecil di MenuPage)
const ShopeeFoodLogo = (props) => (
  <svg {...props} viewBox="0 0 24 24" fill="currentColor">
...
// GrabFood Logo (MENGGANTI DEFINISI DENGAN SVG RESMI DARI USER - GRAB)
const GrabFoodLogo = (props) => (
  <svg {...props} viewBox="0 0 24 24" fill="currentColor">
...
          </div>
        </div>
        {/* Tambahkan komponen peta atau ilustrasi lain di sini jika perlu */}
      </div>
...
            <div className="grid grid-cols-2 gap-4">{filteredOutlets.map((outlet) => (<div key={outlet.id} className="bg-white p-3 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow flex flex-col relative group/card"><div className="aspect-square rounded-xl w-full mb-3 overflow-hidden bg-slate-100 relative z-0"><SmartImage srcOnline={outlet.imageOnline} alt={outlet.name} className="w-full h-full object-cover" />{isAdmin && (<div className="absolute top-2 right-2 z-10 flex gap-1"><button onClick={(e) => { e.stopPropagation(); onRequestEdit(outlet); }} className="bg-yellow-500 text-white p-1.5 rounded-full hover:bg-yellow-600 shadow-md"><EditIcon className="w-3.5 h-3.5" /></button><button onClick={(e) => { e.stopPropagation(); onRequestDelete(outlet); }} className="bg-red-500 text-white p-1.5 rounded-full hover:bg-red-600 shadow-md"><TrashIcon className="w-3.5 h-3.5" /></button></div>)}</div><div className="flex justify-between items-start mb-1 w-full"><h4 className="font-bold text-[#0B2545] text-sm leading-tight truncate pr-1">{outlet.name}</h4><span className={`text-[10px] px-1.5 py-0.5 rounded font-bold flex-shrink-0 ${getBadgeColor(outlet.code)}`}>{outlet.code || "N/A"}</span></div><button onClick={() => onOpenMenu(outlet)} className="mt-auto w-full bg-[#0B2545] text-white text-xs font-bold py-2 rounded-lg hover:bg-[#163A66] transition-colors">Lihat Menu</button></div>))}</div>
          )}
        </div>
...
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.374-5.031c0-5.445 4.429-9.872 9.882-9.872 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.883-9.885 9.883m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.16 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                </svg>
              </div>
              <div>
...
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full flex items-center justify-center shrink-0 bg-[#00AA13]">
                {/* LOGO GOFOOD BARU (Gojek) */}
                <GoFoodLogo className="w-6 h-6" fill="#FFF" />
              </div>
              <div>
...
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-white shrink-0 bg-[#EE4D2D]">
                {/* LOGO SHOPEEFOOD BARU (Shopee) */}
                <ShopeeFoodLogo className="w-5 h-5" fill="#FFF" />
              </div>
              <div>
...
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-white shrink-0 bg-[#00B14F]">
                {/* LOGO GRAB FOOD BARU (Grab) - MENGGUNAKAN SVG RESMI */}
                <GrabFoodLogo className="w-6 h-6" fill="#FFF" />
              </div>
              <div>
